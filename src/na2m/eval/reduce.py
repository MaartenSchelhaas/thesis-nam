...
HARD CONSTRAINTS:
    - STABILITY is computed on the TEST fold (subnet_vectors_test); CONCURVITY on
      the 80% pool (subnet_vectors_pool). These use different stored fields and are
      NOT interchangeable.
    ...
    - Arms are {A, B, C}. There is NO B′ arm (the count-matched control was
      ...
      from B's stored subnet_vectors by B's own contribution ranking — no retraining.
"""
...
def main_effect_instability(measures, arm: str) -> float:
    ...
    Per main term: take each seed's RAW test-fold f-vector (subnet_vectors_test),
    ...
    TODO:
        - Read subnet_vectors_test (NOT pool). For each ("main", k):
...
def interaction_instability(measures, arm: str) -> float:
    ...
    from subnet_vectors_test, centered by their own test-fold means.
    ...
    TODO:
        - Read subnet_vectors_test. Select ("inter", j, k) terms.
...
def concurvity_summary(measures, arm: str) -> dict:
    ...
    on the 80% POOL (subnet_vectors_pool), where "all other" = every main effect +
    ...
    TODO:
        - Read subnet_vectors_pool (NOT test). For each interaction (j,k):
