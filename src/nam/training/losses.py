...
def penalized_loss(
    ...
    Args:
        ...
        fnn_outputs (torch.Tensor): Per-feature outputs stacked by NAM.forward(),
                                    shape (batch_size, num_features).
                                    Used for the output penalty term.
        model (NAM): The NAM instance (needed to iterate parameters for L2).
...
def l2_penalty(model: nn.Module, num_features: int, lambda2: float) -> torch.Tensor:
    ...
    Args:
        model (nn.Module): NAM instance whose parameters are regularised.
